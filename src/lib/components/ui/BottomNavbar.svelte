<script lang="ts">
	...
	const navItems: NavItem[] = [
		...
		{ id: 'transaction', label: 'Transaction', href: '/transactions', icon: 'transaction' },
		{ id: 'settle', label: 'Settle', href: '/settle', icon: 'settle' },
		{ id: 'other', label: 'Other', href: '/settings', icon: 'other' }
	];
		...
		<div class="flex items-center justify-around h-16">
			...
			<a
				...
				class="flex flex-col items-center justify-center gap-0.5 flex-1 py-2	
					{isActive(navItems[0].href) ? 'text-indigo-500' : 'text-slate-400'}"
			>
				<svg class="w-6 h-6" fill={isActive(navItems[0].href) ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
			...
			<a
				...
				class="flex flex-col items-center justify-center gap-0.5 flex-1 py-2
					{isActive(navItems[1].href) ? 'text-indigo-500' : 'text-slate-400'}"
			>
			...
			<a
				...
				class="flex flex-col items-center justify-center gap-0.5 flex-1 py-2
					{isActive(navItems[2].href) ? 'text-indigo-500' : 'text-slate-400'}"
			>
				<svg class="w-6 h-6" fill={isActive(navItems[2].href) ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
			...
			<a
				...
				class="flex flex-col items-center justify-center gap-0.5 flex-1 py-2
					{isActive(navItems[3].href) ? 'text-indigo-500' : 'text-slate-400'}"
			>
				<svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
