...
<div class="space-y-4 pb-20">
        ...
        <div class="space-y-4">
            {#each Array(3) as _}
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-full bg-surface animate-pulse"
                    ></div>
                    <div class="flex-1 space-y-2">
                        <div
                            class="h-4 w-1/3 bg-surface animate-pulse rounded"
                        ></div>
                        <div
                            class="h-3 w-1/4 bg-surface animate-pulse rounded"
                        ></div>
                    </div>
                </div>
            {/each}
    ...
    {:else if transactionsStore.transactions.length === 0}
        <p class="text-muted text-center py-8">Belum ada transaksi.</p>
    {:else}
