<script lang="ts">
  ...
  let categories = $derived(
    categoriesStore.categories.filter((c) => c.type === type),
  );
  ...
  async function handleSubmit() {
    ...
    const data: CreateRecurringTransactionRequest = {
      ...
      subcategory_id:
        type !== "transfer" ? subcategoryId || undefined : undefined,
...
>
  <div class="space-y-4">
    ...
    {#if type === "transfer"}
      ...
      <div class="space-y-4 p-4 bg-muted/30 rounded-xl border border-border/50">
        ...
        <div class="flex justify-center -my-2 relative z-10">
          <div
            class="p-1.5 bg-surface border border-border rounded-full text-muted"
          >
            <svg
              class="w-4 h-4"
              fill="none"
    ...
    <div class="flex gap-3 mt-6">
      <Button variant="secondary" fullWidth onclick={() => (open = false)}
        >Batal</Button
      >
      <Button
        variant="primary"
        fullWidth
        disabled={recurringStore.isLoading ||
          !name ||
          !amount ||
          amount <= 0 ||
          !pocketId ||
          (type === "transfer" && !toPocketId)}
