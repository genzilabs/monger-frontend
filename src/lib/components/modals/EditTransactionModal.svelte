<script lang="ts">
  ...
  const pocketOptions = $derived.by(() => {
    ...
    if (
      ...
      !baseOptions.find((o) => o.value === transaction.related_pocket!.id)
    ) {
      baseOptions.push({
    ...
    if (
      transaction?.pocket &&
      !baseOptions.find((o) => o.value === transaction.pocket!.id)
    ) {
      baseOptions.push({
  ...
  async function handleSubmit() {
    ...
    if (isTransfer) {
      ...
      if (transaction.amount_cents < 0) {
        ...
        const currentRelated =
          transaction.related_pocket_id || transaction.related_pocket?.id;
        if (fromPocketId !== currentRelated)
