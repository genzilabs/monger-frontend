<script lang="ts">
	...
	const colors = ['#448AFF', '#4CAF50', '#FF9800', '#9C27B0', '#F44336', '#00BCD4'];
...
<BottomSheet {open} onClose={handleClose} title="Add New Pocket">
	<div class="space-y-4">
		<!-- Name -->
		<div>
			<label for="pocket-name" class="block text-sm font-medium text-secondary mb-1.5">
				Pocket Name
			</label>
			<input
				id="pocket-name"
		...
		<div>
			...
			<div class="grid grid-cols-2 gap-2">
				{#each pocketTypes as type}
					<button
						type="button"
						class="flex items-center gap-2 p-3 rounded-xl border transition-all {typeSlug === type.slug 
		...
		<!-- Color -->
		<div>
			<label class="block text-sm font-medium text-secondary mb-1.5">Color</label>
			<div class="flex gap-2">
				{#each colors as c}
					<button
						type="button"
						class="w-10 h-10 rounded-full border-2 transition-all"
						style="background: {c}; border-color: {color === c ? 'white' : 'transparent'}; box-shadow: {color === c ? '0 0 0 2px ' + c : 'none'}"
						onclick={() => (color = c)}
					></button>
				{/each}
			</div>
		...
		<!-- Initial Balance -->
		<div>
			<label for="pocket-balance" class="block text-sm font-medium text-secondary mb-1.5">
				Initial Balance
			</label>
			<input
				id="pocket-balance"
	...
	<div class="flex gap-3 mt-6">
		...
		<Button variant="primary" fullWidth loading={isCreating} onclick={handleSubmit}>Add Pocket</Button>
	</div>
</BottomSheet>
