	...
	<div class="flex flex-col gap-4 min-h-[200px]">
		...
		{:else if members.length === 0}
			<div class="flex flex-col items-center justify-center py-8 text-center">
				<div class="w-12 h-12 mb-3 rounded-full bg-muted/20 flex items-center justify-center">
						...
						<div class="flex items-center gap-3">
							<div class="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center">
								{#if member.user_avatar}
									<img src={member.user_avatar} alt="" class="w-10 h-10 rounded-full object-cover" />
								{:else}
							...
							<div class="min-w-0">
								<p class="font-medium text-foreground truncate">{member.user_name || 'Unknown'}</p>
								<p class="text-xs text-muted truncate">{member.user_email || ''}</p>
							</div>
						</div>

						<div class="flex items-center gap-2">
							<span class="text-xs px-2 py-1 rounded-full capitalize {getRoleBadgeClasses(member.role)}">
								{member.role}
							</span>
