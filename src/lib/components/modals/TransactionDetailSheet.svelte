<script lang="ts">
	import { BottomSheet, Button } from '$lib/components/ui';
	...
	let { transaction, currency = 'IDR', open, onClose, onEdit, onDeleted }: Props = $props();
	...
	const typeBgColor = $derived(
		isTransfer ? 'bg-blue-500/10' : isIncome ? 'bg-emerald-500/10' : 'bg-red-500/10'
	);
...
<BottomSheet {open} {onClose} title="Transaction Details">
	{#if transaction}
		<div class="space-y-6">
			<!-- Amount Hero -->
			<div class="text-center py-4">
				<div class="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center {typeBgColor}">
					{#if isTransfer}
						<svg class="w-8 h-8 {typeColor}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
			...
			<div class="space-y-4">
				...
				<div class="flex items-start gap-3">
					...
					<div class="flex-1 min-w-0">
						...
						<p class="text-sm font-medium text-foreground">{transaction.name}</p>
					</div>
				</div>
						...
						<div class="flex-1 min-w-0">
							...
							<p class="text-sm font-medium text-foreground">
								...
								{#if transaction.subcategory}
									<span class="text-muted">→ {transaction.subcategory.name}</span>
								{/if}
							</p>
						</div>
					</div>
			...
			{#if showDeleteConfirm}
				<div class="p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
					<p class="text-sm text-foreground mb-3">
						{#if isTransfer}
							<strong>Warning:</strong> This will delete both the outgoing and incoming transfer transactions and reverse the balance changes.
						{:else}
							Are you sure you want to delete this transaction? The pocket balance will be adjusted.
						{/if}
					</p>
					<div class="flex gap-2">
						<Button variant="secondary" fullWidth onclick={() => showDeleteConfirm = false}>
							Cancel
						</Button>
						<Button variant="danger" fullWidth loading={isDeleting} onclick={handleDelete}>
							Delete
						</Button>
					</div>
				</div>
