<script lang="ts">
	...
	const initialCooldown = $derived(parseInt($page.url.searchParams.get('cooldown') || '60', 10));
	const isPostRegister = $derived($page.url.searchParams.get('postRegister') === 'true');
...
<svelte:head>
	<title>Verification - Monger</title>
</svelte:head>
...
<div class="flex-1 flex flex-col animate-fade-in">
	...
	{#if errorType === 'expired'}
		<div class="bg-red-50 border border-red-200 rounded-2xl p-6 mb-6 text-center">
			...
			<h2 class="text-lg font-semibold text-red-800 mb-1">Code Expired</h2>
			<p class="text-sm text-red-600">
				The verification code sent to {maskedIdentifier} has expired.
			</p>
		</div>
	{/if}
	...
	<!-- Header Card -->
	<Card class="mb-6">
		<h1 class="text-xl font-semibold text-center text-slate-900 mb-2">
			{errorType === 'expired' ? 'Request a New Code' : 'Enter Verification Code'}
		</h1>

		<p class="text-center text-slate-500 text-sm">
			{#if errorType === 'expired'}
				Click the button below to receive a new code.
			{:else}
				We sent a 6-digit code to<br />
				<span class="font-medium text-slate-900">{maskedIdentifier}</span>
			{/if}
		</p>
	</Card>
	...
	<Card class="space-y-6">
		<div class="flex-between text-sm">
			<span class="text-label">Enter 6-digit code</span>
		...
		<p class="text-xs text-center text-slate-400">
			Trying to auto-fill from messages...
		</p>
	</Card>
	...
	<div class="mt-6 space-y-3">
		<div class="flex gap-3">
			<div class="flex-1 flex-center gap-2 py-3 bg-slate-50 rounded-xl">
				<svg class="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
				</svg>
				<span class="text-slate-600 font-mono">{formatTime(cooldown)}</span>
			</div>
		...
		<button onclick={() => goto('/login')} class="btn-action">
			<div class="flex items-center gap-3">
				...
				<span class="text-slate-900">Wrong number?</span>
			</div>
			<svg class="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
		...
		<button class="btn-action">
			<div class="flex items-center gap-3">
				...
				<span class="text-slate-900">Contact Support</span>
			</div>
			<svg class="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
				<path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
			</svg>
